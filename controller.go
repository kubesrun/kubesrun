/*
Copyright 2017 The Kubesrun Authors.
...
import (
	...
	sourcev1alpha1 "kubesrun.top/source-controller/pkg/apis/sourcecontroller/v1alpha1"
	clientset "kubesrun.top/source-controller/pkg/generated/clientset/versioned"
	sourcescheme "kubesrun.top/source-controller/pkg/generated/clientset/versioned/scheme"
	informers "kubesrun.top/source-controller/pkg/generated/informers/externalversions/sourcecontroller/v1alpha1"
	listers "kubesrun.top/source-controller/pkg/generated/listers/sourcecontroller/v1alpha1"
)

const controllerAgentName = "source-controller"

const (
	// SuccessSynced is used as part of the Event 'reason' when an Application is synced
	SuccessSynced = "Synced"
	// ErrResourceExists is used as part of the Event 'reason' when an Application fails
...
type Controller struct {
	...
	// sourceclientset is a clientset for our own API group
...
// NewController returns a new source controller
func NewController(
	kubeclientset kubernetes.Interface,
	sourceclientset clientset.Interface,
	...
	// Add source-controller types to the default Kubernetes Scheme so Events can be
	// logged for source-controller types.
	utilruntime.Must(sourcescheme.AddToScheme(scheme.Scheme))
	...
	// owned by an Application resource then the handler will enqueue that Application resource for
...
func (c *Controller) syncHandler(key string) error {
	...
	deploymentName := application.Spec.DeploymentName
	if deploymentName == "" {
	...
	deployment, err := c.deploymentsLister.Deployments(application.Namespace).Get(deploymentName)
	// If the resource doesn't exist, we'll create it
	if errors.IsNotFound(err) {
		deployment, err = c.kubeclientset.AppsV1().Deployments(application.Namespace).Create(context.TODO(), newDeployment(application), metav1.CreateOptions{})
	}
	...
	if application.Spec.Replicas != nil && *application.Spec.Replicas != *deployment.Spec.Replicas {
		klog.V(4).Infof("Application %s replicas: %d, deployment replicas: %d", name, *application.Spec.Replicas, *deployment.Spec.Replicas)
		deployment, err = c.kubeclientset.AppsV1().Deployments(application.Namespace).Update(context.TODO(), newDeployment(application), metav1.UpdateOptions{})
	}
	...
	err = c.updateApplicationStatus(application, deployment)
	if err != nil {
		return err
	}
...
func (c *Controller) updateApplicationStatus(application *sourcev1alpha1.Application, deployment *appsv1.Deployment) error {
	...
	_, err := c.sourceclientset.SourceV1alpha1().Applications(application.Namespace).UpdateStatus(context.TODO(), applicationCopy, metav1.UpdateOptions{})
	return err
}

// enqueueApplication takes an Application resource and converts it into a namespace/name
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		// If this object is not owned by an Application, we should not do anything more
		// with it.
		...
		if err != nil {
			klog.V(4).Infof("ignoring orphaned object '%s/%s' of application '%s'", object.GetNamespace(), object.GetName(), ownerRef.Name)
			return
		}
...
// newDeployment creates a new Deployment for an Application resource. It also sets
// the appropriate OwnerReferences on the resource so handleObject can discover
// the Application resource that 'owns' it.
func newDeployment(application *sourcev1alpha1.Application) *appsv1.Deployment {
	...
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      application.Spec.DeploymentName,
			Namespace: application.Namespace,
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(application, sourcev1alpha1.SchemeGroupVersion.WithKind("Application")),
			},
		},
